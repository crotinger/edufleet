@php
    // Shared renderer for pre/post-trip inspection result lists.
    // Caller passes `$results` as a Collection of *InspectionResult rows
    // (each row has category_snapshot, description_snapshot, was_critical,
    // result, and optional comment).
    $byCategory = collect($results)->groupBy('category_snapshot', preserveKeys: true);
@endphp
                ...
                <ul style="list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.25rem;">
                    @foreach ($items as $item)
                        @php
                            ...
                            $bg = $item->result === 'fail' ? 'rgb(254 226 226 / 0.3)' : 'transparent';
                        ...
                        <li style="display: flex; align-items: flex-start; gap: 0.75rem; padding: 0.375rem 0.5rem; background: {{ $bg }}; border-radius: 0.25rem; font-size: 0.875rem;">
                            <span style="min-width: 4.5rem; color: {{ $color }}; font-weight: 700; font-size: 0.75rem; padding-top: 0.1rem;">{{ $label }}</span>
